...
# I named a new dataset named fema_count, which organizes the frequency of each natural disasters that occurred in each states
fema_count <- fema_df |> group_by(region, disaster_group, season) |> summarize(count = n(), .groups = "drop")
# Using this fema_count and ggplot, I plotted a heatmap. Each cell has different colors depending on its frequency It has one panel for each season,
ggplot(fema_count, aes(x = region, y = disaster_group, fill = count)) + geom_tile(color = "white") + facet_wrap(~season, scale = "free") + scale_fill_viridis_c(option = "plasma") + theme_minimal() + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + labs(title = "Frequency of Natural Disasters by Region and Season", x = "Region", y = "Incident Type", caption = "Source: FEMA OpenFEMA API, Disaster Declarations Summaries (v2), accessed Dec 13, 2025.", fill = "Count")
